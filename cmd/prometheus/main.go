...
func (c *flagConfig) setFeatureListOptions(logger *slog.Logger) error {
	for _, f := range c.featureList {
		for o := range strings.SplitSeq(f, ",") {
			...
			case "openmetrics-2-0":
				config.ScrapeProtocolsHeaders[config.OpenMetricsText2_0_0] = "application/openmetrics-text;version=2.0.0"
				logger.Info("Experimental OpenMetrics 2.0 scraping enabled. Add OpenMetricsText2.0.0 to scrape_protocols to request it from targets.")
			default:
